...
func init() {
	lib.RegisterCreateServer("boltv2", NewBoltServer)
}
...
func (s *MockBoltServer) mux(req *boltv2.Request) (result *ResponseConfig) {
...
func (s *MockBoltServer) handle(buf buffer.IoBuffer) *ResponseToWrite {
	...
	engine := xprotocol.GetProtocol(boltv2.ProtocolName)
	cmd, err := engine.Decode(ctx, buf)
	if cmd == nil || err != nil {
		return nil
	}
	req, ok := cmd.(*boltv2.Request)
	if !ok {
		return nil
	}
	// select mux

...
func (c *ResponseConfig) HandleRequest(req *boltv2.Request, engine api.XProtocol) (resp api.XRespFrame, status int16) {
	switch req.CmdCode {
...
func (cond *Condition) Match(req *boltv2.Request) bool {
	if cond == nil {
		return true
	}
...
// ResponseBuilder builds a boltv2 response based on the boltv2 request
...
func (b *ResponseBuilder) Build(req *boltv2.Request) (*boltv2.Response, int16) {
	reqId := uint32(req.GetRequestId())
	cmd := boltv2.NewRpcResponse(reqId, uint16(b.StatusCode), protocol.CommonHeader(b.Header), nil)
	if len(b.Body) > 0 {
...
func buildRequest(id uint32, header map[string]string, body []byte, timeout time.Duration) (api.HeaderMap, buffer.IoBuffer) {
	...
	req := boltv2.NewRpcRequest(id, protocol.CommonHeader(header), buf)
	if timeout > 0 {

...
func (d *DeserializeTRHessian) SerializeConnRequestBytes(strEchoBytes []byte) int64 {
	...
	log.DefaultLogger.Debugf("Serialize TR's Communication Request Data")
	...
	log.DefaultLogger.Debugf(string(strEchoBytes))
	...
	if err != nil {
		...
		if so, ok := obj.(reflect.Value); ok {
			...
			log.DefaultLogger.Debugf("TR Request ID is: ", u1.Ctx.Id)
			return u1.Ctx.Id
		}
...
func (d *DeserializeTRHessian) SerializeConnResponseBytes(strEchoBytes []byte) int64 {
	...
	log.DefaultLogger.Debugf("Serialize TR's Communication Response Data")
...
func (d *DeserializeTRHessian) SerializeAppRequestBytes(strEchoBytes []byte) string {
	...
	log.DefaultLogger.Debugf("Serialize TR's Application Request Data")
	...
	if err != nil {
		...
		if so, ok := obj.(reflect.Value); ok {
			...
			log.DefaultLogger.Debugf("TR Request TargetServiceUniqueName  is: ", u1.TargetServiceUniqueName)

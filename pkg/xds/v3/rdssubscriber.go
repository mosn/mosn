...
func (c *ADSClient) reqRoutes(streamClient envoy_service_discovery_v3.AggregatedDiscoveryService_StreamAggregatedResourcesClient) error {
	if streamClient == nil {
		return errors.New("stream client is nil")
	}
	rs := getResponseRequestInfo(EnvoyRoute)
	rs.ResourceNames = rds.GetRouterNames()
	if len(rs.ResourceNames) < 1 {
...
func (c *ADSClient) handleRoutesResp(resp *envoy_service_discovery_v3.DiscoveryResponse) []*envoy_config_route_v3.RouteConfiguration {

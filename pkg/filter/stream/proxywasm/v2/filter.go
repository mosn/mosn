...
type Filter struct {
	...
	contextID       int32
	vmContextID     int32
	pluginContextID int32
	streamContextID int32
	httpContextID   int32
...
func NewFilter(ctx context.Context, pluginName string, rootContextID int32, factory *FilterConfigFactory) *Filter {
	...
	if pluginWrapper == nil {
		log.DefaultLogger.Errorf("[proxywasm020][filter] NewFilter wasm plugin not exists, plugin name: %v", pluginName)
		return nil
	}
	...
	filter.abi = abi.GetABI(instance, proxywasm.ProxyWasmABI_0_2_0)
	if filter.abi == nil {
		log.DefaultLogger.Errorf("[proxywasm020][filter] NewFilter abi not found in instance")
	...
	if filter.exports == nil {
		log.DefaultLogger.Errorf("[proxywasm020][filter] NewFilter fail to get exports part from abi")
		plugin.ReleaseInstance(instance)
	...
	_, err := filter.exports.ProxyOnContextCreate(filter.contextID, filter.rootContextID, proxywasm.ContextTypeVmContext)
	if err != nil {
		log.DefaultLogger.Errorf("[proxywasm020][filter] NewFilter fail to create context id: %v, rootContextID: %v, err: %v",
			filter.contextID, filter.rootContextID, err)
		return nil
	}

	filter.vmContextID, filter.pluginContextID, filter.streamContextID, filter.httpContextID = filter.contextID, filter.contextID, filter.contextID, filter.contextID
...
func (f *Filter) OnDestroy() {
	f.destroyOnce.Do(func() {
		...
		if err != nil {
			log.DefaultLogger.Errorf("[proxywasm020][filter] OnDestroy fail to call ProxyOnDone, err: %v", err)
		}
...
func (f *Filter) OnReceive(ctx context.Context, headers api.HeaderMap, buf buffer.IoBuffer, trailers api.HeaderMap) api.StreamFilterStatus {
	...
	if err != nil || action != proxywasm.ActionContinue {
		log.DefaultLogger.Errorf("[proxywasm020][filter] OnReceive call ProxyOnRequestHeaders err: %v", err)
	...
	if buf != nil && buf.Len() > 0 {
		action, err = f.exports.ProxyOnHttpRequestBody(f.contextID, int32(buf.Len()), int32(endOfStream))
		if err != nil || action != proxywasm.ActionContinue {
			log.DefaultLogger.Errorf("[proxywasm020][filter] OnReceive call ProxyOnHttpRequestBody err: %v", err)
			return api.StreamFilterStop
		}
	}

	if trailers != nil {
		action, err = f.exports.ProxyOnHttpRequestTrailers(f.contextID, int32(headerMapSize(trailers)), int32(endOfStream))
		if err != nil || action != proxywasm.ActionContinue {
			log.DefaultLogger.Errorf("[proxywasm020][filter] OnReceive call ProxyOnRequestTrailers err: %v", err)
			return api.StreamFilterStop
		}
	}
...
func (f *Filter) Append(ctx context.Context, headers api.HeaderMap, buf buffer.IoBuffer, trailers api.HeaderMap) api.StreamFilterStatus {
	...
	if err != nil || action != proxywasm.ActionContinue {
		log.DefaultLogger.Errorf("[proxywasm020][filter] Append call ProxyOnResponseHeaders err: %v", err)
		return api.StreamFilterStop
	}
	...
	if buf != nil && buf.Len() > 0 {
		action, err = f.exports.ProxyOnHttpResponseBody(f.contextID, int32(buf.Len()), int32(endOfStream))
		if err != nil || action != proxywasm.ActionContinue {
			log.DefaultLogger.Errorf("[proxywasm020][filter] Append call ProxyOnHttpResponseBody err: %v", err)
			return api.StreamFilterStop
		}
	}

	if trailers != nil {
		action, err = f.exports.ProxyOnHttpResponseTrailers(f.contextID, int32(headerMapSize(trailers)), int32(endOfStream))
		if err != nil || action != proxywasm.ActionContinue {
			log.DefaultLogger.Errorf("[proxywasm020][filter] Append call ProxyOnHttpResponseTrailers err: %v", err)
			return api.StreamFilterStop
		}
	}
...
func (f *Filter) GetHttpRequestHeader() common.HeaderMap {
	...
	return &proxywasm020.HeaderMapWrapper{HeaderMap: f.receiverFilterHandler.GetRequestHeaders()}
}
...
func (f *Filter) GetHttpRequestBody() common.IoBuffer {
	...
	return &proxywasm020.IoBufferWrapper{IoBuffer: f.receiverFilterHandler.GetRequestData()}
}
...
func (f *Filter) GetHttpRequestTrailer() common.HeaderMap {
	...
	return &proxywasm020.HeaderMapWrapper{HeaderMap: f.receiverFilterHandler.GetRequestTrailers()}
}
...
func (f *Filter) GetHttpResponseHeader() common.HeaderMap {
	...
	return &proxywasm020.HeaderMapWrapper{HeaderMap: f.senderFilterHandler.GetResponseHeaders()}
}
...
func (f *Filter) GetHttpResponseBody() common.IoBuffer {
	...
	return &proxywasm020.IoBufferWrapper{IoBuffer: f.senderFilterHandler.GetResponseData()}
}
...
func (f *Filter) GetHttpResponseTrailer() common.HeaderMap {
	...
	return &proxywasm020.HeaderMapWrapper{HeaderMap: f.senderFilterHandler.GetResponseTrailers()}
}

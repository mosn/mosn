...
func createProxyWasmFilterFactory(conf map[string]interface{}) (api.StreamFilterChainFactory, error) {
	...
	if err != nil {
		log.DefaultLogger.Errorf("[proxywasm020][factory] createProxyWasmFilterFactory fail to parse config, err: %v", err)
		return nil, err
	}
	...
	if config.FromWasmPlugin == "" {
		...
		if err != nil {
			log.DefaultLogger.Errorf("[proxywasm020][factory] createProxyWasmFilterFactory fail to add plugin, err: %v", err)
			return nil, err
		}
	} else {
...
func (f *FilterConfigFactory) CreateFilterChain(context context.Context, callbacks api.StreamFilterChainFactoryCallbacks) {
	filter := NewFilter(context, f.pluginName, f.config.VmID, f)
	if filter == nil {
		return
	}
...
func (f *FilterConfigFactory) GetVmID() int32 {
	return f.config.VmID
}
...
func (f *FilterConfigFactory) OnPluginStart(plugin types.WasmPlugin) {
	plugin.Exec(func(instance types.WasmInstance) bool {
		a := abi.GetABI(instance, proxywasm.ProxyWasmABI_0_2_0)
		a.SetABIImports(f)
		...
		var err error
		_, err = exports.ProxyOnContextCreate(f.config.VmID, 0, proxywasm.ContextTypeVmContext)
		if err != nil {
			log.DefaultLogger.Errorf("[proxywasm020][factory] OnPluginStart fail to create root context id, err: %v", err)
			return true
		}
		...
		_, err = exports.ProxyOnVmStart(f.config.VmID, int32(vmConfigSize))
		if err != nil {
			log.DefaultLogger.Errorf("[proxywasm020][factory] OnPluginStart fail to create vm id, err: %v", err)
			return true
		}
		...
		f.pluginId = f.config.VmID

		_, err = exports.ProxyOnConfigure(f.pluginId, int32(pluginConfigSize))
		if err != nil {
			log.DefaultLogger.Errorf("[proxywasm020][factory] OnPluginStart fail to create plugin id, err: %v", err)
			return true
		}

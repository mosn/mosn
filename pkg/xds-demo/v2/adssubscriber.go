...
type TypeURLHandleFunc func(*ADSClient, *envoy_api_v2.DiscoveryResponse)
...
const (
	...
	// For MOSNG
	AliPaySofaGateway = "alipay.com/mesh.api.v2.SofaGateway"
)

func init() {
	typeURLHandleFuncs = make(map[string]TypeURLHandleFunc)
	RegisterTypeURLHandleFunc(EnvoyListener, EnvoyListenerHandler)
	RegisterTypeURLHandleFunc(EnvoyCluster, EnvoyClusterHandler)
	RegisterTypeURLHandleFunc(EnvoyClusterLoadAssignment, EnvoyClusterLoadAssignmentHandler)
	RegisterTypeURLHandleFunc(EnvoyRouteConfiguration, EnvoyRouteConfigurationHandler)
	// For MOSNG
	RegisterTypeURLHandleFunc(AliPaySofaGateway, AliPaySofaGatewayHandler)
}
...
func EnvoyListenerHandler(adsClient *ADSClient, resp *envoy_api_v2.DiscoveryResponse) {
	...
	// TODO: OnAddOrUpdateListeners will removed from MosnConfig
...
func EnvoyClusterHandler(adsClient *ADSClient, resp *envoy_api_v2.DiscoveryResponse) {
	...
	// TODO: OnUpdateClusters will removed from MosnConfig
...
func EnvoyClusterLoadAssignmentHandler(adsClient *ADSClient, resp *envoy_api_v2.DiscoveryResponse) {
	...
	// TODO: OnUpdateEndpoints will removed from MosnConfig
...
func EnvoyRouteConfigurationHandler(adsClient *ADSClient, resp *envoy_api_v2.DiscoveryResponse) {
	...
	// TODO: OnAddOrUpdateRouters  will removed from MosnConfig
	adsClient.MosnConfig.OnAddOrUpdateRouters(routes)

}

func AliPaySofaGatewayHandler(adsClient *ADSClient, resp *envoy_api_v2.DiscoveryResponse) {
	// Gateway logic
	// MosnConfig.OnXXX will changed to OnXXX (not related to MosnConfig)
}
...
func (adsClient *ADSClient) receiveThread() {
	for {
		...
		default:
			...
			if handleFunc, ok := typeURLHandleFuncs[typeURL]; ok {
				handleFunc(adsClient, resp)
			}
		}
	}
}

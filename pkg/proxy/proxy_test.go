...
func TestNewProxy(t *testing.T) {
	...
	t.Run("config with proxy general HTTP1", func(t *testing.T) {
		...
		_ = variable.SetVariable(ctx, types.VariableProxyGeneralConfig, extConfig)
		pv := NewProxy(ctx, cfg)
		// verify
		p := pv.(*proxy)
		var v http.StreamConfig
		if pgc, err := variable.GetVariable(p.context, types.VariableProxyGeneralConfig); err == nil {

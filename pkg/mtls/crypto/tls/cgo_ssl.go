...
func (ctx *SslCtx) serverSslCtxInit(config *Config) error {
	...
	C.ssl_ctx_set_cert_verify_callback_serverCertVerifyCallBack(sslCtx, configPtr)
...
func (ctx *SslCtx) clientSslCtxInit(config *Config) error {
	...
	//actually we use self-define go verify process to hook openssl
	//verify process, so we always set SSL_VERIFY_PEER
	...
	configPtr := cgoPointerSave(config)
	C.ssl_ctx_set_cert_verify_callback_clientCertVerifyCallBack(sslCtx, configPtr)

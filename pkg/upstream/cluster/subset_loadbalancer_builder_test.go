...
func TestSubsetLbBuilder_InitIndex(t *testing.T) {
	...
	builder := NewSubsetLoadBalancerBuilder(&clusterInfo{}, createHostset(hosts), subsetMergeKeys(subsetInfo.SubsetKeys(), subsetInfo.DefaultSubset()))
...
func TestSubsetLbBuilder_ExtractKvComb(t *testing.T) {
	subsetInfo := NewLBSubsetInfo(exampleSubsetConfig())
	builder := NewSubsetLoadBalancerBuilder(&clusterInfo{}, createHostset(exampleHostConfigs()), subsetMergeKeys(subsetInfo.SubsetKeys(), subsetInfo.DefaultSubset()))

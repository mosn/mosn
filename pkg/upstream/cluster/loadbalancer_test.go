...
func TestEdfWRRLB(t *testing.T) {
	...
	hs := &hostSet{}
	hs.setFinalHost(hosts)
	lb := rrFactory.newRoundRobinLoadBalancer(nil, hs)
	total := 1000000
	runCase := func(subTotal int) {
		results := map[string]int{}
		for i := 0; i < subTotal; i++ {
...
func BenchmarkEdfWRRLbSimple(b *testing.B) {
	pool := makePool(4)
	hosts := []types.Host{}
	for i := 0; i < 4; i++ {
		host := &mockHost{
...
func BenchmarkEdfWRRLbMultiple(b *testing.B) {
	testCases := []struct {
		name       string
		count      int
		max_weight int
...
func BenchmarkEdfWRRLbParallel(b *testing.B) {
	testCases := []struct {
		name       string
		count      int
		max_weight int

...
func TestProxyLog(t *testing.T) {
	...
	_ = variable.SetVariable(ctx, types.VarTraceId, traceId)
	_ = variable.SetVariable(ctx, types.VarConnectionID, connId)
	_ = variable.SetVariable(ctx, types.VarUpstreamConnectionID, upstreamConnID)
...
func BenchmarkProxyLog(b *testing.B) {
	...
	ctx := variable.NewVariableContext(context.Background())
	_ = variable.SetVariable(ctx, types.VarTraceId, "0abfc19515355177863163255e6d87")

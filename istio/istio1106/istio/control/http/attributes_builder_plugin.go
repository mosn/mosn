...
import "mosn.io/mosn/istio/istio1106/istio/utils"
...
func RegisterAttributesBuilderPlugin(plugin AttributesBuilderPlugin) {
	attributesBuilderPlugins = append(attributesBuilderPlugins, plugin)
}

func AddAttributesByPlugins(builder *utils.AttributesBuilder) {
	for _, plugin := range attributesBuilderPlugins {
		plugin.AddAttributes(builder)
	}
}

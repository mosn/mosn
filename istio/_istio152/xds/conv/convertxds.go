...
func ConvertListenerConfig(xdsListener *xdsapi.Listener, rh routeHandler) *v2.Listener {
	...
	listenerConfig.FilterChains, rawSelectedFilters = convertFilterChainsAndGetRawFilter(xdsListener, rh)
	...
	for _, xdsFilterChain := range xdsListener.GetFilterChains() {
		for _, xdsFilter := range xdsFilterChain.GetFilters() {
			if value, ok := httpBaseConfig[xdsFilter.GetName()]; ok && value {
				...
				//	}
				//}
			} else if xdsFilter.GetName() == iv2.MIXER {
				// support later
				return nil
			} else {
...
func convertFilterChainsAndGetRawFilter(xdsListener *xdsapi.Listener, rh routeHandler) ([]v2.FilterChain, *xdslistener.Filter) {
	if xdsListener == nil {
		return nil, nil
	}
	...
	return []v2.FilterChain{{
		FilterChainConfig: v2.FilterChainConfig{
			...
			Filters:          convertFilters(oneFilter, rh),
		},
...
func convertFilters(xdsFilters *xdslistener.Filter, rh routeHandler) []v2.Filter {
	if xdsFilters == nil {
		return nil
	}
	...
	filterMaps := convertFilterConfig(xdsFilters, rh)
...
// TODO: more filter config support
func convertFilterConfig(filter *xdslistener.Filter, rh routeHandler) map[string]map[string]interface{} {
	if filter == nil {
		return nil
	}
...
func convertPerRouteConfig(xdsPerRouteConfig map[string]*any.Any) map[string]interface{} {
	...
	for key, config := range xdsPerRouteConfig {
		...
		case iv2.MIXER:
			...
			if err != nil {
				log.DefaultLogger.Infof("convertPerRouteConfig[%s] error: %v", iv2.MIXER, err)
				continue
			}

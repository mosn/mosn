...
func (cvt *xdsConverter) ConvertAddOrUpdateRouters(routers []*envoy_api_v2.RouteConfiguration) {
...
func (cvt *xdsConverter) listenerRouterHandler(isRds bool, routerConfig *v2.RouterConfiguration) {
	if routerConfig == nil {
		log.DefaultLogger.Errorf("[xds] [router handler] no router config found")
		return
	}
...
func (cvt *xdsConverter) ConvertAddOrUpdateListeners(listeners []*envoy_api_v2.Listener) {
	for _, listener := range listeners {
		log.DefaultLogger.Debugf("xds convert listener config: %+v", listener)

		mosnListener := ConvertListenerConfig(listener, cvt.listenerRouterHandler)
		if mosnListener == nil {
...
func (cvt *xdsConverter) ConvertDeleteListeners(listeners []*envoy_api_v2.Listener) {
	for _, listener := range listeners {
		mosnListener := ConvertListenerConfig(listener, cvt.listenerRouterHandler)
		if mosnListener == nil {
			continue
...
func (cvt *xdsConverter) ConvertUpdateClusters(clusters []*envoy_api_v2.Cluster) {
...
func (cvt *xdsConverter) ConvertDeleteClusters(clusters []*envoy_api_v2.Cluster) {
	mosnClusters := ConvertClustersConfig(clusters)
...
func (cvt *xdsConverter) ConvertUpdateEndpoints(loadAssignments []*envoy_api_v2.ClusterLoadAssignment) error {
	var errGlobal error
